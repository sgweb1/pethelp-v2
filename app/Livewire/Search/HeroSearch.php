...
class HeroSearch extends Component
{
    ...
    {
        // Initialize with cached popular locations
    ...
    {
        $cacheKey = 'hero_search_categories_' . ($this->serviceType ?: 'all');

        return Cache::remember($cacheKey, 3600, function () {
            return MapItem::select('category_name as category')
                ->distinct()
                ->whereNotNull('category_name')
                ->where('status', 'published')
                ->when($this->serviceType, function ($query) {
                    $query->where('content_type', $this->serviceType);
                })
    ...
    public function updatedLocation(): void
    {
        ...
        if ($this->showSuggestions) {
            $this->isLoading = true;
            // Trigger suggestion computation
    ...
    {
        // Pre-warm cache with popular locations for better UX
    ...
    {
        return view('livewire.search.hero-search');
    }
}