        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 sm:p-6">
            ...
            <div class="relative mb-2">
                ...
                <input
                    ...
                    @input="radiusLabel = parseInt($event.target.value) + ' km'"
                    @change="updateRadius(parseInt($event.target.value))"
                    min="0"
                    max="60"
                    step="1"
            ...
            {{-- Radius Impact Info - DANE Z GUS --}}
            <div class="mt-4 grid grid-cols-2 gap-3">
                <div class="bg-gradient-to-br from-emerald-50 to-green-50 rounded-xl p-3 border border-emerald-200">
                    <div class="text-xs text-emerald-700 mb-1 flex items-center justify-between gap-1">
                        <div class="flex items-center gap-1">
                            <span>📊</span>
                            <span>Potencjalni klienci w zasięgu:</span>
                        </div>
                        @if($latitude && $longitude)
                            <button
                                type="button"
                                wire:click="refreshEstimation"
                                wire:loading.attr="disabled"
                                wire:target="calculatePotentialClients,refreshEstimation"
                                class="text-emerald-600 hover:text-emerald-700 transition-colors disabled:opacity-50"
                                title="Odśwież estymację"
                            >
                                <svg wire:loading.remove wire:target="calculatePotentialClients,refreshEstimation" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                </svg>
                                <svg wire:loading wire:target="calculatePotentialClients,refreshEstimation" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                    ...
                    <div class="text-lg font-bold text-emerald-600" wire:loading.remove wire:target="calculatePotentialClients,refreshEstimation,updatedServiceRadius,updatedLatitude,updatedLongitude">
                        @if($estimatedClients > 0)
                            ~{{ number_format($estimatedClients, 0, ',', ' ') }}
                        @else
                            <span class="text-gray-400">Wybierz lokalizację</span>
                        @endif
                    </div>
                    <div class="text-sm text-emerald-500" wire:loading wire:target="calculatePotentialClients,refreshEstimation,updatedServiceRadius,updatedLatitude,updatedLongitude">
                        <span class="animate-pulse">Obliczanie...</span>
                    </div>
                    @if($estimatedClients > 0)
                    <div class="text-xs text-emerald-600 mt-1 opacity-75">
                        Dane z Eurostat 2021
                    </div>
                    @endif
                </div>
                <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl p-3 border border-blue-200">
                    <div class="text-xs text-blue-700 mb-1 flex items-center gap-1">
                        <span>🚗</span>
                        <span>Średni czas dojazdu:</span>
                    </div>
